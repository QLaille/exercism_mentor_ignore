...
EXERCISM_HOME="$(exercism workspace)"
# Sanity Checks
if [ "$EXERCISM_HOME" == "" ]
	then echo "ERROR!!!!! Exercism CLI is not installed, please install"
	exit
fi

if [ ! -d ${EXERCISM_HOME}'/users/*' ]
    then echo "ERROR!!!!! No exercise folder exist, please download an exercise"
    exit
fi

EXERCISE_HOME=${EXERCISM_HOME}/users/*/*/*
bash ./exercism_mentor_ignore.sh comment ${EXERCISE_HOME}/test/*.c
make -C ${EXERCISE_HOME}
code -w ${EXERCISE_HOME}/src/*.c
rm -rf ${EXERCISM_HOME}/users/*